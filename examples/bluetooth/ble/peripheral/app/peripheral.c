...
static const uint8_t conn_interval_data[] = {
...
// Scan Response Data elements
static const struct bt_data scan_rsp[] = {
    BT_DATA(GAP_ADTYPE_LOCAL_NAME_COMPLETE, DEVICE_NAME, DEVICE_NAME_LEN),
    BT_DATA(GAP_ADTYPE_SLAVE_CONN_INTERVAL_RANGE, conn_interval_data, sizeof(conn_interval_data)),
    BT_DATA_BYTES(GAP_ADTYPE_POWER_LEVEL, 0)
};

// Advertisement Data elements
static const struct bt_data adv_data[] = {
    BT_DATA_BYTES(GAP_ADTYPE_FLAGS, 
                  DEFAULT_DISCOVERABLE_MODE | GAP_ADTYPE_FLAGS_BREDR_NOT_SUPPORTED),
    BT_DATA_BYTES(GAP_ADTYPE_16BIT_MORE,
...
void Peripheral_Init()
{
    ...
    // Convert bt_data structures to LTV format
    size_t scan_rsp_len = bt_data_to_ltv(scanRspData, sizeof(scanRspData), 
                                          scan_rsp, sizeof(scan_rsp) / sizeof(scan_rsp[0]));
    size_t adv_data_len = bt_data_to_ltv(advertData, sizeof(advertData), 
                                          adv_data, sizeof(adv_data) / sizeof(adv_data[0]));
    ...
    // Setup the GAP Peripheral Role Profile
    {
        ...
        GAPRole_SetParameter(GAPROLE_SCAN_RSP_DATA, scan_rsp_len, scanRspData);
        GAPRole_SetParameter(GAPROLE_ADVERT_DATA, adv_data_len, advertData);
...
static void Peripheral_ProcessGAPMsg(gapRoleEvent_t *pEvent)
{
    ...
    case GAP_SCAN_REQUEST_EVENT: {
        WCH_LOGD("Receive scan req from: %02X:%02X:%02X:%02X:%02X:%02X", pEvent->scanReqEvt.scannerAddr[0],
    ...
    case GAP_PHY_UPDATE_EVENT: {
        WCH_LOGI("Phy update Rx:%x Tx:%x ..", pEvent->linkPhyUpdate.connRxPHYS, pEvent->linkPhyUpdate.connTxPHYS);
        break;
    }
...
static void Peripheral_ProcessTMOSMsg(tmos_event_hdr_t *pMsg)
{
    ...
    case GATT_MSG_EVENT: {
        ...
        if (pMsgEvent->method == ATT_MTU_UPDATED_EVENT) {
            ...
            WCH_LOGI("mtu exchange: %d", pMsgEvent->msg.exchangeMTUReq.clientRxMTU);
        }
        break;
    }
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    if (peripheralConnList.connHandle != GAP_CONNHANDLE_INIT) {
        ...
        WCH_LOGI("Conn %x - Int %x ", event->connectionHandle, event->connInterval);
    }
}
...
static void peripheralRssiCB(uint16_t connHandle, int8_t rssi)
{
    WCH_LOGI("RSSI -%d dB Conn  %x ", -rssi, connHandle);
}
                                    ...
                                    uint16_t connTimeout)
{
    if (connHandle == peripheralConnList.connHandle) {
        ...
        WCH_LOGI("Update %x - Int %x ", connHandle, connInterval);
    } else {
        WCH_LOGI("ERR..");
    }
}
...
static void peripheralStateNotificationCB(gapRole_States_t newState, gapRoleEvent_t *pEvent)
{
    ...
    case GAPROLE_STARTED:
        WCH_LOGI("Initialized..");
        break;

    case GAPROLE_ADVERTISING:
        if (pEvent->gap.opcode == GAP_LINK_TERMINATED_EVENT) {
            Peripheral_LinkTerminated(pEvent);
            WCH_LOGI("Disconnected.. Reason:%x", pEvent->linkTerminate.reason);
            WCH_LOGI("Advertising..");
    ...
    case GAPROLE_CONNECTED_ADV:
        if (pEvent->gap.opcode == GAP_MAKE_DISCOVERABLE_DONE_EVENT) {
            WCH_LOGI("Connected Advertising..");
        }
        break;

    case GAPROLE_WAITING:
        if (pEvent->gap.opcode == GAP_END_DISCOVERABLE_DONE_EVENT) {
            WCH_LOGI("Waiting for advertising..");
            ...
            WCH_LOGI("Error..%x", pEvent->gap.opcode);
        }
        break;
...
static void simpleProfileChangeCB(uint8_t paramID, uint8_t *pValue, uint16_t len)
{
    ...
    case SIMPLEPROFILE_CHAR1: {
        ...
        WCH_LOGI("profile ChangeCB CHAR1.. ");
        break;
    }
    ...
    case SIMPLEPROFILE_CHAR3: {
        ...
        tmos_memcpy(newValue, pValue, len);
        WCH_LOGI("profile ChangeCB CHAR3..");
        break;
    }
