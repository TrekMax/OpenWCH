...
#define RF_MODE_TX_RX    1 // 0: TX  1: RX
...
void RF_2G4StatusCallBack(uint8_t sta, uint8_t crc, uint8_t *rxBuf)
{
    ...
    case TX_MODE_RX_DATA: {
        ...
        if (crc == 0) {
            ...
            WCH_LOG_PRINTF("tx recv,rssi:%d\n", (int8_t)rxBuf[0]);
            WCH_LOG_PRINTF("len:%d-", rxBuf[1]);

            for (i = 0; i < rxBuf[1]; i++) {
                WCH_LOG_PRINTF("%x ", rxBuf[i + 2]);
            }
            WCH_LOG_PRINTF("\n");
        } else {
            if (crc & (1 << 0)) {
                WCH_LOG_PRINTF("crc error\n");
            }

            if (crc & (1 << 1)) {
                WCH_LOG_PRINTF("match type error\n");
            }
        }
        break;
    }
    ...
    case TX_MODE_HOP_SHUT: {
        tx_end_flag = TRUE;
        WCH_LOG_PRINTF("TX_MODE_HOP_SHUT...\n");
    ...
    case RX_MODE_HOP_SHUT: {
        WCH_LOG_PRINTF("RX_MODE_HOP_SHUT...\n");
        rx_end_flag = TRUE;
...
void RF_Init(void)
{
    ...
    WCH_LOG_PRINTF("rf 2.4g init: %x\n", state);

#if (RF_MODE_TX_RX == 1)
    {
        // RX mode
        WCH_LOG_PRINTF("RX mode...\n");
        tmos_set_event(taskID, SBP_RF_CHANNEL_HOP_RX_EVT);
    }
#else
    RF_Wait_Rx_End();
